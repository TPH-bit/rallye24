...
export default function App(){
  ...
  async function signUp(e){
    ...
    if (!user) { alert('Compte créé. Vérifiez l’email si nécessaire.'); return }
    const { error: upErr } = await supabase.from('profiles')
      .update({ team_name: teamName || 'Équipe' }).eq('id', user.id)
    if (upErr) await supabase.from('profiles')
  ...
  return (
    <div className="wrap">
      ...
      <div className="row">
        ...
        <button onClick={loadRiddles} disabled={loading}>{loading ? 'Chargement…' : t('load_riddles')}</button>
      </div>
